...
int Pathfinder::getValidCorner(int cornerIdx) {
  ...
  if (cornerIdx == 0) {
    ...
    targetX = sim->gridWidth - 1;
    targetZ = 0;
  } // NE
  else if (cornerIdx == 2) {
    targetX = 0;
    targetZ = sim->gridHeight - 1;
  } // SW
  else if (cornerIdx == 3) {
    targetX = sim->gridWidth - 1;
    targetZ = sim->gridHeight - 1;
  ...
  for (int z = 0; z < sim->gridHeight; ++z) {
    for (int x = 0; x < sim->gridWidth; ++x) {
      ...
      if (sim->heightGrid[idx].height > -9000.0f) {
        float dist =
            std::sqrt(std::pow(x - targetX, 2) + std::pow(z - targetZ, 2));
        if (dist < minDist) {
          minDist = dist;
          bestIdx = idx;
        }
...
std::vector<glm::vec3> Pathfinder::findPathAStar(int startIdx, int targetIdx,
                                                 float snowLevel) {
  ...
  while (!openSet.empty()) {
    ...
    for (int i = 0; i < 8; ++i) {
      ...
      if (nx >= 0 && nx < w && nz >= 0 && nz < h) {
        ...
        if (sim->heightGrid[neighbor].height < -9000.0f)
          continue;
...
void Pathfinder::generatePath(int cornerIdx, float snowLevel) {
  std::cout << "Generating path from corner " << cornerIdx << " with snowLevel "
            << snowLevel << std::endl;

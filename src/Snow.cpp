...
bool SnowSimulation::barycentric(glm::vec2 p, glm::vec2 a, glm::vec2 b, glm::vec2 c, float &u, float &v, float &w) {
...
void SnowSimulation::initGridFromModel(const obj::Model& model, const glm::mat4& modelMatrix) {
    ...
    for (size_t i = 0; i < model.vertex.size(); i += 3) {
        glm::vec4 pos(model.vertex[i], model.vertex[i+1], model.vertex[i+2], 1.0f);
        pos = modelMatrix * pos;
        ...
        for (size_t i = 0; i < indices.size(); i += 3) {
            ...
            for (int gz = minGz; gz <= maxGz; ++gz) {
                for (int gx = minGx; gx <= maxGx; ++gx) {
                    ...
                    float u, v, w;
                    if (barycentric(glm::vec2(cx, cz), glm::vec2(v0.x, v0.z), glm::vec2(v1.x, v1.z), glm::vec2(v2.x, v2.z), u, v, w)) {
...
void SnowSimulation::initRendering(GLuint shaderProgram) {
    ...
    glBufferData(GL_ARRAY_BUFFER, renderData.size() * sizeof(glm::vec3), nullptr, GL_DYNAMIC_DRAW);
...
void SnowSimulation::render(const glm::mat4& view, const glm::mat4& projection) {
    ...
    glBindBuffer(GL_ARRAY_BUFFER, vbo);
    glBufferSubData(GL_ARRAY_BUFFER, 0, activeParticleCount * sizeof(glm::vec3), renderData.data());

    glEnable(GL_PROGRAM_POINT_SIZE);
    glEnable(GL_BLEND);
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    
    // Depth test enabled, but don't write depth for particles to avoid ugly artifacts
